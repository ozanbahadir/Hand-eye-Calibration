...
def data_Noise():
    data = list()
    #with open(str(mypath) + "/Result/H3Translation.txt",'r') as f:
    with open(str(mypath) + "/Result/H2TranslationWithNoise.txt", 'r') as f:
    #with open(str(mypath) + "/Result/H3Orientation.txt", 'r') as f:
        c=0
        for line in f:
            linedata = line.split()
...
for index,i in enumerate(b):
    plt.plot(x, i, label="test without noise" )
